...
function render() {
    ...
    byId('rows').innerHTML=rows.slice(offset,offset+pageSize).map(p=>{const cat=category(p),[catIcon,catLabel]=categoryMeta(cat),state=opportunityState(p),[stateIcon,stateLabel]=stateMeta(state);return `<article class="event-card state-${escape(state.toLowerCase())}">
      ...
      <div class="event-overview"><div class="event-identity"><h3>${escape(displayGame(p))}</h3><p class="event-selection">${escape(p.pick)}</p><p class="event-market">${escape(p.market)} <span>·</span> ${escape((p.marketSignal||'—').replaceAll('_',' '))}</p>${p.drawEstimation ? `<p class="event-market">Empate estimado: ${escape(p.drawEstimation.probability)}% · margen supuesto ${escape(p.drawEstimation.assumedMarginPct)}%. Estimación sin calibrar.</p>` : ''}</div>
