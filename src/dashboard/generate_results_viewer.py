...
.panel{padding:14px;margin-bottom:18px}.filters{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:8px}.filters input,.filters select,.filters button{width:100%;min-width:0;padding:9px 10px;border:1px solid var(--border);border-radius:8px;background:var(--panel2);color:var(--text)}.filters button{cursor:pointer;font-weight:700}.filter-wide{grid-column:span 2}.check{display:flex!important;align-items:center;justify-content:center;gap:6px;white-space:nowrap}.check input{width:auto}.summary{font-size:12px;color:var(--muted);margin-top:10px}.summary b{color:var(--teal)}
...
<input id="search" class="filter-wide" placeholder="Buscar evento, pick o liga">
...
<select id="stake"><option value="">Todos los stakes</option><option value="1">Stake 1u</option><option value="1.5">Stake 1.5u</option><option value="2">Stake 2u</option><option value="2.5">Stake 2.5u</option><option value="3">Stake 3u</option><option value="3.5">Stake 3.5u</option><option value="4">Stake 4u</option><option value="4.5">Stake 4.5u</option><option value="5">Stake 5u</option></select>
<input id="modelMin" type="number" min="0" max="100" step="1" placeholder="Modelo mín. %">
<input id="edgeMin" type="number" min="0" step="0.1" placeholder="Edge mín. %">
<input id="evMin" type="number" min="0" step="0.1" placeholder="EV mín. %">
<select id="result"><option value="SETTLED" selected>Solo liquidados</option><option value="">Todos los estados</option><option>WIN</option><option>HALF_WIN</option><option>LOSS</option><option>HALF_LOSS</option><option>PUSH</option><option>VOID</option><option>PENDING</option><option>REVIEW</option></select>
...
function apply(){const q=$('search').value.toLowerCase().trim(),from=$('dateFrom').value,to=$('dateTo').value,league=$('league').value,cat=$('category').value,res=$('result').value,free=$('freeOnly').checked,stake=$('stake').value,modelMin=$('modelMin').value,edgeMin=$('edgeMin').value,evMin=$('evMin').value,today=isoDate(new Date());scoped=PICKS.filter(p=>{const blob=`${p.game||''} ${p.pick||''} ${p.league||''} ${p.market||''}`.toLowerCase(),date=p.date||'';return date<=today&&num(p.ev)>0&&num(p.modelEdge)>0&&(!q||blob.includes(q))&&(!from||date>=from)&&(!to||date<=to)&&(!league||p.league===league)&&(!cat||p.pickCategory===cat)&&(!stake||num(p.stake)===num(stake))&&(!modelMin||num(p.modelProb)>=num(modelMin))&&(!edgeMin||num(p.modelEdge)>=num(edgeMin))&&(!evMin||num(p.ev)>=num(evMin))&&(!free||p.freeRelease)});filtered=scoped.filter(p=>!res||(res==='SETTLED'?finalSet.has(p.result):p.result===res));page=1;render()}
function metrics(){const settled=filtered.filter(p=>finalSet.has(p.result)),wins=settled.filter(p=>p.result==='WIN').length+settled.filter(p=>p.result==='HALF_WIN').length*.5,losses=settled.filter(p=>p.result==='LOSS').length+settled.filter(p=>p.result==='HALF_LOSS').length*.5,push=settled.filter(p=>p.result==='PUSH').length,units=settled.reduce((s,p)=>s+num(p.profitUnits),0),risk=settled.filter(p=>p.result!=='VOID').reduce((s,p)=>s+num(p.stake),0),pending=scoped.filter(p=>p.result==='PENDING').length,review=scoped.filter(p=>p.result==='REVIEW').length,roi=risk?units/risk*100:0,wr=wins+losses?wins/(wins+losses)*100:0;$('kSettled').textContent=settled.length;$('kTotalSub').textContent=`${filtered.length} visibles`;$('kRecord').textContent=`${wins}-${losses}-${push}`;$('kUnits').textContent=`${units>=0?'+':''}${fmt(units)}u`;$('kRoi').textContent=`${roi>=0?'+':''}${fmt(roi)}%`;$('kRisked').textContent=`${fmt(risk)}u apostadas`;$('kWinRate').textContent=`${fmt(wr)}%`;$('kPending').textContent=pending+review;$('kReview').textContent=`${review} revisión · ${pending} en curso`;$('kUnits').className='kpi-value '+(units>=0?'positive':'negative');$('kRoi').className='kpi-value '+(roi>=0?'positive':'negative')}
