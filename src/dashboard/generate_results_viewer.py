...
<div class="kpi"><div class="kpi-label">Pendientes</div><div id="kPending" class="kpi-value">0</div><div id="kReview" class="kpi-sub">0 requieren revisión</div></div>
</section>
...
<select id="result"><option value="">Todos los resultados</option><option>WIN</option><option>LOSS</option><option>PUSH</option><option>VOID</option><option>PENDING</option><option>REVIEW</option></select>
...
<script>const PICKS=__PICKS_JSON__;let filtered=[],page=1;const PAGE_SIZE=50;let unitsChart,resultChart;
...
function apply(){const q=$('search').value.toLowerCase().trim(),from=$('dateFrom').value,to=$('dateTo').value,league=$('league').value,cat=$('category').value,res=$('result').value,free=$('freeOnly').checked;filtered=PICKS.filter(p=>{const blob=`${p.game||''} ${p.pick||''} ${p.league||''} ${p.market||''}`.toLowerCase();return(!q||blob.includes(q))&&(!from||(p.date||'')>=from)&&(!to||(p.date||'')<=to)&&(!league||p.league===league)&&(!cat||p.pickCategory===cat)&&(!res||p.result===res)&&(!free||p.freeRelease)});page=1;render()}
function metrics(){const settled=filtered.filter(p=>finalSet.has(p.result)),wins=settled.filter(p=>p.result==='WIN').length,losses=settled.filter(p=>p.result==='LOSS').length,push=settled.filter(p=>p.result==='PUSH').length,units=settled.reduce((s,p)=>s+num(p.profitUnits),0),risk=settled.filter(p=>p.result!=='VOID').reduce((s,p)=>s+num(p.stake),0),pending=filtered.filter(p=>p.result==='PENDING').length,review=filtered.filter(p=>p.result==='REVIEW').length,roi=risk?units/risk*100:0,wr=wins+losses?wins/(wins+losses)*100:0;$('kSettled').textContent=settled.length;$('kTotalSub').textContent=`${filtered.length} históricos`;$('kRecord').textContent=`${wins}-${losses}-${push}`;$('kUnits').textContent=`${units>=0?'+':''}${fmt(units)}u`;$('kRoi').textContent=`${roi>=0?'+':''}${fmt(roi)}%`;$('kRisked').textContent=`${fmt(risk)}u apostadas`;$('kWinRate').textContent=`${fmt(wr)}%`;$('kPending').textContent=pending;$('kReview').textContent=`${review} requieren revisión`;$('kUnits').className='kpi-value '+(units>=0?'positive':'negative');$('kRoi').className='kpi-value '+(roi>=0?'positive':'negative')}
...
function detail(label,value){return `<div class="detail"><span>${esc(label)}</span><b>${esc(value)}</b></div>`}function openDetail(p){const s=p.settlement||{},e=p.eventLookup||{},snaps=Array.isArray(p.qualificationSnapshots)?p.qualificationSnapshots:[];$('modalTitle').textContent=p.pick||'Detalle';$('modalBody').innerHTML=`<div class="modal-grid">${detail('Evento',p.game)}${detail('Liga',p.league)}${detail('Resultado',p.result)}${detail('Balance',p.profitUnits==null?'—':fmt(p.profitUnits)+'u')}${detail('Primera calificación',p.firstQualifiedAt)}${detail('Última versión viable',p.lastQualifiedAt)}${detail('Fuente',s.source||'—')}${detail('ESPN Event ID',s.eventId||e.eventId||'—')}${detail('Marcador',score(p))}${detail('Notas',s.notes||'—')}${detail('Archivo',p.historyFile)}${detail('Snapshots',snaps.length)}</div>${snaps.length?`<div class="snapshots"><h3>Evolución mientras conservó valor</h3>${snaps.map(x=>`<div class="snapshot"><span>${esc(x.observedAt)}</span><span>Cuota ${esc(x.odds)}</span><span>EV ${fmt(x.ev)}%</span><span>Edge ${fmt(x.modelEdge)}%</span><span>${fmt(x.stake,1)}u</span><span>${esc(x.marketSignal)}</span></div>`).join('')}</div>`:''}`;$('detailModal').showModal()}
function render(){$('shown').textContent=filtered.length;$('total').textContent=PICKS.length;metrics();renderRows();charts()}['search','dateFrom','dateTo','league','category','result','freeOnly'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',apply));$('clear').onclick=()=>{['search','dateFrom','dateTo','league','category','result'].forEach(id=>$(id).value='');$('freeOnly').checked=false;apply()};$('prev').onclick=()=>{page--;renderRows();scrollTo(0,0)};$('next').onclick=()=>{page++;renderRows();scrollTo(0,0)};$('closeModal').onclick=()=>$('detailModal').close();$('detailModal').onclick=e=>{if(e.target===$('detailModal'))$('detailModal').close()};$('theme').onclick=()=>{document.documentElement.dataset.theme=document.documentElement.dataset.theme==='light'?'dark':'light';charts()};populate();filtered=[...PICKS];render();</script>
</body></html>'''
